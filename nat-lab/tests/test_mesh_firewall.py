...
@pytest.mark.asyncio
async def test_mesh_firewall_tcp_last_ack_state() -> None:
    async with AsyncExitStack() as exit_stack:
        api = API()
        (alpha, beta) = api.default_config_two_nodes()
        ...
        client_beta = await exit_stack.enter_async_context(
            telio.Client(
                connection_beta, beta, telio.AdapterType.BoringTun
        ...
        # kill server and check on client conntrack
        # conntrack show LAST_ACK -> TIME_WAIT
        # netstat -t -n -p shows empty
        async def netstat_on_stdout(stdout: str) -> None:
            for line in stdout.splitlines():
                print(f"[beta] [netstat] {line}")
                if (
                    CLIENT_BETA_IP in line
                    and f"{CLIENT_ALPHA_IP}:{str(PORT)}" in line
                    and "LAST_ACK" in line
                ):
                    assert False

        await connection_beta.create_process(["netstat", "-p", "-t", "-n"]).execute(
            stderr_callback=netstat_on_stdout, stdout_callback=netstat_on_stdout
        )
